...
logging.info("Downloading %s to %s", s3_raw_images, local_zip_dir + local_raw_images_zip)
...
logging.info("Downloading %s to %s", s3_cropped_images, local_zip_dir + local_cropped_images_zip)
s3.download_file(s3_bucket_name, s3_cropped_images, local_zip_dir + local_cropped_images_zip)

# Unzip Files
logging.info("Unzipping files")
with zipfile.ZipFile(local_zip_dir + local_raw_images_zip, 'r') as zip:
  zip.extractall(local_raw_dir)

with zipfile.ZipFile(local_zip_dir + local_cropped_images_zip, 'r') as zip:
  zip.extractall(local_raw_dir)
